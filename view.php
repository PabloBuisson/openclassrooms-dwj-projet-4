<?php
    ...
    $req = $bdd->prepare('SELECT id_article, pseudo, comment, DATE_FORMAT(date_comment, "Publié le %d/%m/%Y à %H:%i:%s") AS date_com FROM comments WHERE id_article = ? ORDER BY date_comment DESC');
    $req->execute(array($_GET['id']));
?>
...
<head>
    ...
    <title><?= $article['title']; ?> | Le site officiel de Jean Forteroche</title>
        ...
        <div class="row">
            <div class="col-lg-8 offset-lg-2">
                ...
                <?php while ($comments = $req->fetch()) { ?>
                    <p class="text-center"><strong><?= $comments['pseudo']; ?></strong> : <?= $comments['comment']; ?> <small class="text-muted"><?= $comments['date_com']; ?></small></p>
                <?php } ?>
            </div>
        </div>
    </div>
